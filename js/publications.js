...
function renderPublications() {
  ...
  sorted.forEach(p => {
    ...
    tags.appendChild(el('span', 'tag tag-outline', String(p.year)));
    tags.appendChild(el('span', 'tag tag-neutral', p.type));
    if (p.citedBy) tags.appendChild(el('span', 'tag tag-accent', `피인용 ${p.citedBy}회`));
    item.appendChild(tags);
    const titleHtml = p.link
      ? `<a href="${p.link}" target="_blank" rel="noopener">${p.title}</a>`
      : p.title;
    item.appendChild(el('p', 'item-title', titleHtml));
    item.appendChild(el('p', 'item-meta', `${p.authors.join(', ')} · ${p.venue}`));
